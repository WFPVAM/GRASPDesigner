...
public class WeeklyReminder extends Reminder {
	...
	public WeeklyReminder(long startdate, long enddate, Type type, String recipients, String subject, String content) {
		super(startdate, enddate, type, recipients, subject, content);
	}
	
	@Override
	public String getOccurrenceLabel() {
		return InternationalisationUtils.getI18nString(RemindersConstants.WEEKLY);
	}
	...
	public void run() {
		...
		if (now.after(end)) {
			...
			this.refreshReminder();
				  ...
				  now.get(Calendar.DAY_OF_WEEK) == start.get(Calendar.DAY_OF_WEEK) &&
				  	(now.get(Calendar.WEEK_OF_YEAR) >= start.get(Calendar.WEEK_OF_YEAR) ||
