...
/**Models a JTextField allowing only numbers.
 * 
...
{  
    ...
    /**THe document that actually filters the alphanumeric value not accepted
     * 
    ...
    {  
        ...
        {  
            ...
            {  
                ...
                //return;  Commented because it did not permit negative char '-'
            }  
  
            if(text.startsWith(""+NEGATIVE) )  
            {  
            	String cut = text.substring(1); 
            	if (cut.indexOf(NEGATIVE) != -1) //Check to ensure there is only one negative char.
            		return;
                if( !allowNegative )  
                    return;  
                else  
