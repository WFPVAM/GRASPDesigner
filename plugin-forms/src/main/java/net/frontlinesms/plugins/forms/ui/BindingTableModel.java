 ...
 */
public class BindingTableModel extends AbstractTableModel implements TableModelListener {
	private ArrayList<PreviewComponent> components;
	private ArrayList<BindingContainer> bindType;
	private String[] columnNames = new String[] { "Component", "Type", "Value" };
	...
	public Object getValueAt(int row, int column) {
		 ...
		 */
		if (column == 0) {
			return components.get(row).getComponent().getName();
		} 
		if (column == 1)
	...
	public void setValueAt(Object aValue, int row, int column) {
		if (column == 0) {
			//components.set(row, (PreviewComponent) aValue);
			this.changeComponentInBinding((PreviewComponent) aValue, row);
		}
		if (column != 2) 
			super.setValueAt(aValue, row, column);
		else {
			String sValue = (String) aValue;
			//System.out.println("setvalueat: " +sValue);
			//super.setValueAt(aValue, row, column);
	...
	public void updateBindingType(BindType newType, int row) {
		...
		this.bindType.set(row, newTypeContainer);
	...
	public void updateBindingValue(String newValue, int row) {
		...
		this.bindType.set(row, newValueContainer);
		FormsUiController.getInstance().bindChanged(pComponent, oldValueContainer, newValueContainer);
		fireTableDataChanged();
	}
	
	/**Fabaris_a.zanchi
	 * Method invoked when a component of a binding is changed.
	 * 
	 * @param newComp The new component who will be involved in the binding.
	 * @param row
	 */
	public void changeComponentInBinding(PreviewComponent newComp, int row) {
		PreviewComponent oldComp = components.get(row);
		BindingContainer bCont = bindType.get(row);
		FormsUiController.getInstance().bindChangedComponent(newComp, bCont, oldComp);
		components.set(row, newComp);
	}
