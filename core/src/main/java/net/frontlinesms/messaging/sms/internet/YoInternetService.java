...
public class YoInternetService extends AbstractSmsInternetService {
	...
	try {
		...
		return ((FrontlineMessage) nodeList.item(0)).getTextContent();
	    }
	...
	try {
		...
		LOG.debug("Authenticate Error: " + ((FrontlineMessage) nodeList.item(0)).getTextContent());
		return false;
	    }
	    nodeList = doc.getElementsByTagName(ACCOUNT_TAG);
	    if (((FrontlineMessage) nodeList.item(0)).getTextContent().equals(accountNumber)) {
		nodeList = doc.getElementsByTagName(AUTHENTICITY_TAG);
		if (((FrontlineMessage) nodeList.item(0)).getTextContent().equals("VALID")) {
		    return true;
		}
	    } else {
		return false;
