 ...
 * 
 * Built for the not-for-profit sector, to provide -> Group messaging ->
 * Auto-responders -> SMS data gathering
 * 
 * This system is built to provide out of the box auto-detection of standard GSM
 * phones for sending messages, and also to use a built-in or external database
 * for storing the results.
 * 
 * The architecture is built so that it can be easily extended.
 * 
 * The default usage is as a desktop application, looking after the phones and
 * providing the GUI interface at the same time, but it should be
 * straightforward to separate it into a server and client interface, or even a
 * web driven interface if that would be required.
 * 
 * The architecture is as follows:
 * 
 * FrontlineSMS.java - starts the application and passes messages and events
 * around the system. SmsHandler.java - runs as a separate thread, will create
 * and manage GSM phone handlers and also internet SMS messaging centres
 * 
 * see {@link "http://www.frontlinesms.net"} for more details. copyright owned
 * by Kiwanja.net
 * 
...
public class FrontlineSMS implements SmsSender, SmsListener, EmailListener, EventObserver {
	...
	// > INSTANCE VARIABLES
	/** Fabaris_a.zanchi Software Version */
	private String internalSoftwareVersion;
	// > DATA ACCESS OBJECTS
	...
	/** Data Access Object for {@link User_Credential}s */
	private UserDao userDao;
	/** Data Access Object for {@link DataTransfer}s */
	private DataTransferDao dataTransferDao;
	/**
	 * Fabaris_a.zanchi Data Access Object for {@link Form}s
	 */
	private FormDao formDao;
	/**
	 * Fabaris_a.zanchi Data Access Object for {@link FormReponse}s
	 */
	private FormResponseDao formResponseDao;
	/**
	 * Fabaris_a.zanchi Data Access Object for {@link SurveyDao}s
	 */
	private SurveyDao surveyDao;
	/**
	 * Fabaris_Raji  Data Access Object for {@link StatisticsDao}s
	 */
	...
	private UserListener userlListener;
	...
	public void initApplicationContext() {
		...
		// Create a base ApplicationContext defining the hibernate config file
		// we need to import
		...
		String databaseExternalConfigPath = ResourceUtils.getConfigDirectoryPath() + ResourceUtils.PROPERTIES_DIRECTORY_NAME
				+ File.separatorChar + appProperties.getDatabaseConfigPath();
		String[] configLocations = getSpringConfigLocations(databaseExternalConfigPath);
		baseApplicationContext.refresh();

		FileSystemXmlApplicationContext applicationContext = new FileSystemXmlApplicationContext(configLocations, false,
				baseApplicationContext);
		this.applicationContext = applicationContext;

		// Add post-processor to handle substituted database properties
		PropertyPlaceholderConfigurer propertyPlaceholderConfigurer = new PropertyPlaceholderConfigurer();
		String databasePropertiesPath = ResourceUtils.getConfigDirectoryPath() + ResourceUtils.PROPERTIES_DIRECTORY_NAME
		...
		formResponseDao = (FormResponseDao) applicationContext.getBean("formResponseDao");
		surveyDao = (SurveyDao) applicationContext.getBean("surveyDao");
		eventBus = (EventBus) applicationContext.getBean("eventBus");
		// Added By Fabaris_Raji
		userDao = (UserDao) applicationContext.getBean("userDao");
		dataTransferDao = (DataTransferDao) applicationContext.getBean("dataTransferDao");
		statisticsDao = (StatisticsDao) applicationContext.getBean("statisticsDao");
		configurationDao = (ConfigurationDao) applicationContext.getBean("configurationDao");
		
		this.internalSoftwareVersion = AppProperties.getInstance().getInternalVersion();
	...
	/**
	 * Start services for the {@link FrontlineSMS} instance. N.B. this must be
	 * called <strong>after</strong> {@link #initApplicationContext()} This
	 * method should be called only once before the services are stopped using
	 * {@link #stopServices()}.
	 */
	public void startServices() {
		...
		// We need to reload all messages, which status is OUTBOX, to the
		// outbox.
	...
	/**
	 * Shutdown and re-initialise the {@link #applicationContext}. TODO it has
	 * not been confirmed this method exectutes cleanly, so it should be used
	 * with caution.
	 */
	public void reboot() {
	...
	/**
	 * Gives all receiving e-mail accounts to the {@link MmsServiceManager} so
	 * it can use them as {@link PopImapEmailMmsReceiver}s
	 */
	...
	/**
	 * Create the configLocations bean for the Hibernate config. This method
	 * should only be called from within
	 * {@link FrontlineSMS#initApplicationContext()}
	 * 
	 * @return {@link BeanDefinition} containing details of the hibernate config
	 *         for the app and its plugins.
	 */
	...
	/**
	 * Gets a list of configLocations required for initialising Spring
	 * {@link ApplicationContext}. This method should only be called from within
	 * {@link FrontlineSMS#initApplicationContext()}
	 * 
	 * @param externalConfigPath
	 *            Spring path to the external Spring database config file
	 * @return list of configLocations used for initialising
		...
		if (this.smsDeviceEventListener != null) {
			...
			if (status == SmsModemStatus.TRY_TO_CONNECT) {
				...
				if (settings != null) {
					...
					// Only set the PIN number if it hasn't been set in the
	...
	public void sendMessage(FrontlineMessage message) {
		...
		// Fabaris_A.zanchi insert here code to generate zipped-base64 encoded
		// message
		  
		
		if(message.getTextContent().startsWith("<response><data>")){
			
			try {
				String responsemessage = getEncodedZippedContent(message.getTextContent());
				 message.setTextMessageContent(responsemessage);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				LOG.debug("Error setting zipped and encoded message content: " + e.getMessage());
			}
			
		}else{
			try {
				String[]   messagestring=	message.getTextContent().split("<__>");
		
				String messagecodec = getEncodedZippedContent(messagestring[1]);		
		        message.setTextMessageContent(messagestring[0]+"<__>"+messagecodec);
			} catch (Exception e) {
				LOG.debug("Error setting zipped and encoded message content: " + e.getMessage());
			}
		}
	...
	/**
	 * Sends an SMS using the phoneManager in the standard way. The only
	 * advantage this method provides over using the phoneManager is that this
	 * will redirect emulator messages in the correct manner.
	 * 
	 * @param targetNumber
	...
	public FrontlineMessage sendTextMessage(String targetNumber, String textContent) {
		...
		// By default, currently replaces the name by the phone number if it's
		// not in the contacts
		...
		textContent = MessageFormatter.formatMessage(textContent, MessageFormatter.MARKER_RECIPIENT_NAME, recipientName,
				MessageFormatter.MARKER_RECIPIENT_NUMBER, targetNumber);

		FrontlineMessage m;
		if (targetNumber.equals(FrontlineSMSConstants.EMULATOR_MSISDN)) {
			m = FrontlineMessage.createOutgoingMessage(System.currentTimeMillis(), FrontlineSMSConstants.EMULATOR_MSISDN,
					FrontlineSMSConstants.EMULATOR_MSISDN, textContent.trim());
			m.setStatus(Status.SENT);
			messageDao.saveMessage(m);
			outgoingMessageEvent(EMULATOR, m);
			incomingMessageEvent(EMULATOR, new CIncomingMessage(System.currentTimeMillis(), FrontlineSMSConstants.EMULATOR_MSISDN,
					textContent.trim(), 1, "NYI"));
		} else {
	...
	// > ACCESSOR METHODS
	/** @return {@link #dataTransferDao} Added by Fabaris_Raji */
	public DataTransferDao getDataTransferDao() {
		return this.dataTransferDao;
	}

	/** @return {@link #userDao} Added by Fabaris_Raji */
	...
	/** Fabaris_a.zanchi returns the current software version */
	public String getSoftwareVersion() {
		return this.internalSoftwareVersion;
	}
	 ...
	 *            new value for
	 *            {@link #smsDeviceEvent(SmsService, SmsServiceStatus)}
	 */
	...
	/**
	 * @param emailListener
	 *            new value for {@link #emailListener}
	 */
	...
	/**
	 * Adds another {@link IncomingMessageListener} to
	 * {@link IncomingMessageProcessor}.
	 * 
	 * @param incomingMessageListener
	 *            new {@link IncomingMessageListener}
	...
	/**
	 * Removes a {@link IncomingMessageListener} from
	 * {@link IncomingMessageProcessor}.
	 * 
	 * @param incomingMessageListener
	 *            {@link IncomingMessageListener} to be removed
	...
	/**
	 * Handles the way statistics are loaded or not at startup. TODO this method
	 * should not be here - most likely should be in
	 * {@link UiGeneratorController}
	 * 
	 * @param uiController
	 */
...
//		}
	...
	public boolean shouldLaunchStatsCollection() {
		...
		if (dateLastPrompt == null) {
			// This is the first time we are checking if the dialog must be
			// prompted, this should then be the first launch.
			// We set the last prompt date to the current date to delay the
			// prompt until STATISTICS_DAYS_BEFORE_RELAUNCH days of use.
			appProperties.setLastStatisticsPromptDate();
			appProperties.saveToDisk();

			return false;
		} else {
			long dateNextPrompt = dateLastPrompt
					+ (FrontlineSMSConstants.MILLIS_PER_DAY * FrontlineSMSConstants.STATISTICS_DAYS_BEFORE_RELAUNCH);
	...
	public int getNumberOfActiveConnections() {
		return this.smsServiceManager.getNumberOfActiveConnections() + this.mmsServiceManager.getNumberOfActiveConnections();
	}
	...
	public void notify(FrontlineEventNotification notification) {
		if (notification instanceof MmsReceivedNotification) {
			...
			if (entity instanceof EmailAccount && ((EmailAccount) entity).isForReceiving()) {
				// If there is any change in the E-Mail accounts, we refresh the
				// list of MmsEmailServices
	...
	/**
	 * A.zanchi zips and base64-encodes the textmessage
	 * 
	 * @param textContent
	 *            String of text content of message
	 * @return
	 * @throws SAXException
	 ...
	 */
	/*
	 * private static String getEncodedFastSetContent(String textContent) throws
	 * Exception { InputStream xmlDocument = new
	 * ByteArrayInputStream(textContent.getBytes()); ByteArrayOutputStream
	 * fastDocument = new ByteArrayOutputStream();
	 * 
	 * // Create Fast Infoset SAX serializer SAXDocumentSerializer
	 * saxDocumentSerializer = new SAXDocumentSerializer(); // Set the output
	 * stream saxDocumentSerializer.setOutputStream(fastDocument);
	 * 
	 * // Instantiate JAXP SAX parser factory SAXParserFactory saxParserFactory
	 ...
	 * 
	 * saxParserFactory.setNamespaceAware(true); // Instantiate the JAXP SAX
	 * parser SAXParser saxParser = saxParserFactory.newSAXParser(); // Set the
	 * lexical handler
	 * saxParser.setProperty("http://xml.org/sax/properties/lexical-handler",
	 * saxDocumentSerializer); // Parse the XML document and convert to a fast
	 * infoset document saxParser.parse(xmlDocument, saxDocumentSerializer);
	 * 
	 * return Base64Utils.encode(fastDocument.toByteArray());
	 * 
	 * // return fastDocument.toString("UTF-8"); }
	 */

	/**
	 * Fabaris_a.zanchi Zip a message and convert it into base64
	 * 
	 * @param textContent
	 *            literal string content of message
	 * @return a string base64-encoded containing the gzipped message
	 * @throws Exception
	 */
	private static String getEncodedZippedContent(String textContent) throws Exception {
		ByteArrayInputStream inStream = new ByteArrayInputStream(textContent.getBytes("UTF-8"));
		...
		while ((i = inStream.read(buffer)) > 0) {
			gzipOutstream.write(buffer, 0, i);
		}
