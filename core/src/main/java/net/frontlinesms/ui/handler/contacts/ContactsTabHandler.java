...
//> CONSTRUCTORS
	...
	public ContactsTabHandler(UiGeneratorController ui) {
		...
		this.formDao = ui.getFrontlineController().getFormDao();
...
//> UI METHODS
	...
	public void showDeleteOptionDialog() {
		...
		long numberOfForms = countAllForms(g);
		...
		if (!this.ui.isDefaultGroup(g)) {
			if (groupMembershipDao.getMemberCount(g) > 0) {
				...
				if(numberOfForms == 0){
					choiceDialogHandler.showChoiceDialog(true,true,false,
					...
					choiceDialogHandler.showChoiceDialog(true,true,true,
							"removeSelectedFromGroupList('false', choiceDialog)",
							"removeSelectedFromGroupList('false', choiceDialog)",
							I18N_SENTENCE_DELETE_CONTACTS_FROM_GROUPS);
				}
				...
				if(numberOfForms > 0){
					if(g.getParent() != null){
						choiceDialogHandler.showChoiceDialog(false,false,true,
								"removeSelectedFromGroupList('false', choiceDialog)",
								"removeSelectedFromGroupList('false', choiceDialog)",
								"");
					}else{
	...
	/**
	 * Count all forms starting from g to its descendant nodes
	 * @param g
	 * @return
	 */
	private long countAllForms(Group g){
		long result = this.formDao.countFormsByGroup(g);
//		List<Group> descendantNodes = this.groupDao.getAllDescendantNode(g);
//		this.formDao.countFormsByGroups(descendantNodes);
		return result;
	}
