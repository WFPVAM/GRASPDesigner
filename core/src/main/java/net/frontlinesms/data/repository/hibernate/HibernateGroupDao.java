...
public class HibernateGroupDao extends BaseHibernateDao<Group> implements GroupDao {
	...
	public List<Group> getAllDescendantNode(Group parent){
		List result = null;
		String path = parent.getPath();
		String sql = "from frontline_group g where g.parentPath LIKE '%"+path+"%'";
	...
	public Set<Group> getAllDescendantNodeFromGroups(List<Group> groups){
		...
		for (Group group : groups) {
			result.addAll(getAllDescendantNode(group));
		}
		return result;
	}
	
	public boolean hasDescendantNodes(Group parent){
		String path = parent.getPath();
		String sql = "select count(*) from frontline_group g where g.parentPath LIKE '%"+path+"%'";
		return (Long)getHibernateTemplate().find(sql).get(0) != 0;
	}
