...
public abstract class BaseHibernateDao<E> extends HibernateDaoSupport {
	...
	protected E saveAndReturnEntityWithoutDuplicateHandling(E entity) {
		log.trace("Saving entity: " + entity);
		this.getHibernateTemplate().save(entity);
		log.trace("Entity saved.");
		eventBus.notifyObservers(new EntitySavedNotification<E>(entity));
...
//	public void storeFormResponce(FormResponse entity){
//		long id = entity.getId();
//		FormResponseSingleton.getInstance().setId(id);
//		this.getHibernateTemplate().findByNamedQuery("inrange",new Object[]{id,FormResponseSingleton.getInstance().getGeoLocation()});
//	}
	...
	/**
	 * Saves an entity .
	 * @return the entity already stored
	 * @param entity entity to save 
	 * @throws DuplicateKeyException if there was a {@link ConstraintViolationException} thrown while saving
	 */
	
	protected E saveAndReturnEntity(E entity) throws DuplicateKeyException {
		try {
			return saveAndReturnEntityWithoutDuplicateHandling(entity);
		} catch(RuntimeException ex) {
			if(isClashOfUniqueColumns(ex)) {
				throw new DuplicateKeyException(ex);
			} else {
