...
public class MOService {
	...
	LOG.info("Status: " + ((FrontlineMessage) nodeList.item(0)).getTextContent());
	if (((FrontlineMessage) nodeList.item(0)).getTextContent().equals("ERROR")) {
	    // Ignoring error message?
	    nodeList = doc.getElementsByTagName(ERROR_MESSAGE);
	    LOG.debug("MO Error: " + ((FrontlineMessage) nodeList.item(0)).getTextContent());
	    LOG.info("[DEBUG] MO Error: " + ((FrontlineMessage) nodeList.item(0)).getTextContent());
	    return null;
	}
	nodeList = doc.getElementsByTagName(RETRIEVED_COUNT_TAG);
	LOG.info(((FrontlineMessage) nodeList.item(0)).getTextContent());
	...
	for (int i = 0; i < messages; i++) {
		...
		if (nodeName.equalsIgnoreCase(TEXT_TAG)) {
		    messageDetail[3] = ((FrontlineMessage) nodeList.item(j)).getTextContent();
		} else if (nodeName.equalsIgnoreCase(SENDER_TAG)) {
		    messageDetail[1] = ((FrontlineMessage) nodeList.item(j)).getTextContent();
		} else if (nodeName.equalsIgnoreCase(DESTINATION_TAG)) {
		    messageDetail[2] = ((FrontlineMessage) nodeList.item(j)).getTextContent();
		} else if (nodeName.equalsIgnoreCase(DATE_TAG)) {
		    messageDetail[0] = ((FrontlineMessage) nodeList.item(j)).getTextContent();
		}
	    }
	    long epoch = 0;
	    try {
